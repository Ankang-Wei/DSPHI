...
genes_seq = load_fa(path="data/only-high3-370h.fasta")
#genes_seq = load_fa(path="../data/PHIDetector/host.fasta")
genes_kmers = {}
for gene in genes_seq.keys():
  genes_kmers[gene] = summary_kmers(build_kmers(seq=genes_seq[gene], k_size=3))
...
kmers_freq_frame.to_csv("data/only-host_kmers.csv",index = True, sep=',',header = True,na_rep='0')
